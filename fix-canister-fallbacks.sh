...
echo "🔧 Corrigiendo fallback directo y nullish de process.env.CANISTER_ID en index.js..."

FILES=$(find src/declarations -name index.js)

for file in $FILES; do
  # Fallback backend
  if grep -q 'process.env.CANISTER_ID_HECHOENOAXACA_ICP_BACKEND' "$file"; then
    sed -i \
  ...
  # Fallback frontend
  if grep -q 'process.env.CANISTER_ID_HECHOENOAXACA_ICP_FRONTEND' "$file"; then
    sed -i \
