...
const Products = () => {
    ...
    const updateProduct = async () => {
        ...
        try {
            const imageBlobs = await Promise.all(selectedImages.map((image) => image.arrayBuffer().then((buffer) => new Blob([buffer], { type: image.type }))));
    ...
    return (<>
      {principal ? (<div className="row mt-5">
          <div className="col">
                ...
                <table className="table">
                  ...
                  <tbody>
                    {products.map((product) => (<tr key={product.id}>
                        ...
                        <td>
                          {product.imagenes.length > 0 ? (product.imagenes.map((src, index) => src && (<img key={index} src={src} alt={`Imagen de ${product.nombre}`} style={{ maxWidth: "50px", maxHeight: "50px", marginRight: "5px" }}/>))) : (<span>Sin imagen</span>)}
                        </td>
                        <td>{product.tipo}</td>
                        <td>
                          <button type="button" className="btn btn-primary" onClick={() => handleShowModalEditar(product.id)}>
                            Editar
                          </button>
                        </td>
                        <td>
                          <button type="button" className="btn btn-danger" onClick={() => handleShowModalEliminar(product.id)}>
                            Eliminar
                          </button>
                        </td>
                      </tr>))}
