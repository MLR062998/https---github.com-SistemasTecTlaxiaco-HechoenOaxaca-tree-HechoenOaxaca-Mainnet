...
const NotificacionesCliente = ({ principalId }) => {
    ...
    const fetchNotificaciones = async () => {
        try {
            setLoading(true);
            const notificacionesRes = await HechoenOaxaca_icp_backend.getNotificacionesByCliente(principalId);
    ...
    return (<div className="notificaciones-cliente container mt-4">
      <h2 className="text-center mb-4">Mis Notificaciones</h2>

      {loading ? (<p className="text-center">Cargando notificaciones...</p>) : notificaciones.length > 0 ? (<ListGroup>
          {notificaciones.map((notificacion, index) => (<ListGroup.Item key={index} className="d-flex justify-content-between align-items-start">
              <div>
        ...
        </ListGroup>) : (<p className="text-center">No tienes notificaciones nuevas.</p>)}
    </div>);
};
