...
const Cliente = ({ principalId }) => {
    ...
    const handleSaveChanges = async () => {
        try {
            const result = await HechoenOaxaca_icp_backend.actualizarPerfil(principalId, editFormData.nombreCompleto, editFormData.lugarOrigen, editFormData.telefono);
            if ("ok" in result) {
    ...
    return (<div className="cliente-dashboard">
      ...
      <div className="top-container d-flex justify-content-between align-items-center my-3">
        ...
        <div className="search-filter-container d-flex gap-3">
          <Form.Control type="text" placeholder="Buscar productos..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ width: "300px" }}/>
          <Form.Select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} style={{ width: "200px" }}>
      ...
      {perfil && (<div className="perfil text-center">
          <h4>Mi Perfil</h4>
      ...
      <div className="productos-disponibles">
        ...
        {loading ? (<p className="text-center">Cargando productos...</p>) : (<div className="d-flex flex-wrap justify-content-center gap-3">
            {filteredProducts.map((producto) => (<Card key={producto.id} style={{ width: "18rem" }}>
                <Card.Body>
