...
const Administrador = ({ principalId }) => {
    ...
    const agregarAdministrador = async () => {
        ...
        try {
            setLoading(true);
            const result = await HechoenOaxaca_icp_backend.asignarAdministrador(newAdminPrincipal);
            if (result.ok) {
    ...
    return (<div className="admin-dashboard">
      ...
      <div className="usuarios">
        ...
        {loading ? (<p>Cargando usuarios...</p>) : (<Table striped bordered hover>
            <thead>
              <tr>
                <th>Nombre</th>
                <th>Teléfono</th>
            ...
            <tbody>
              {usuarios.map((usuario) => (<tr key={usuario.principalId}>
      ...
      <div className="nuevo-admin">
        ...
        <input type="text" className="form-control mb-2" placeholder="Principal ID del nuevo administrador" value={newAdminPrincipal} onChange={(e) => setNewAdminPrincipal(e.target.value)}/>
