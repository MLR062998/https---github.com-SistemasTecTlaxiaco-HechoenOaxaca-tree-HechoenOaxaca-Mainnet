...
const Intermediario = ({ principalId }) => {
    ...
    return (<div className="intermediario-dashboard">
      ...
      <div className="pedidos">
        ...
        <Table striped bordered hover>
          ...
          <tbody>
            {pedidos.length > 0 ? (pedidos.map((pedido) => (<tr key={pedido.id}>
                  <td>{pedido.id}</td>
                  <td>{pedido.clienteNombre}</td>
                  <td>{pedido.direccion}</td>
                  <td>{pedido.estado}</td>
                  <td>
                    <Button variant="info" className="me-2" onClick={() => handleShowDetails(pedido)}>
                      Detalles
                    </Button>
                    {pedido.estado === "Pendiente" && (<Button variant="success" onClick={() => marcarComoEntregado(pedido.id)}>
      ...
      <Modal show={showDetailsModal} onHide={handleCloseDetailsModal}>
        ...
        <Modal.Body>
          {selectedPedido ? (<>
              ...
              <ul>
                {selectedPedido.productos.map((producto, index) => (<li key={index}>{producto.nombre} - {producto.cantidad}</li>))}
              </ul>
