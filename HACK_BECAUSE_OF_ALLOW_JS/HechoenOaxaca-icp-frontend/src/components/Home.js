...
const Home = () => {
    ...
    const fetchProducts = async () => {
        ...
        try {
            ...
            const processedProducts = productsRes.map(product => ({
                ...product,
                imagenes: product.imagenes.map(imagen => URL.createObjectURL(new Blob([new Uint8Array(imagen)], { type: "image/jpeg" })))
            }));
  ...
  <div className="container mt-4">
    {loading ? (<p>Cargando productos...</p>) : (<div className="row">
        {products.map((product) => (<div key={product.id} className="col-md-4 mb-4">
            <Card>
              {product.imagenes.length > 0 && (<Card.Img variant="top" src={product.imagenes[0]} // Muestra la primera imagen
                 alt={`Imagen de ${product.nombre}`} style={{ maxHeight: "200px", objectFit: "cover" }}/>)}
              <Card.Body>
      ...
      <Compra show={showModal} onClose={() => setShowModal(false)} product={selectedProduct} onPurchase={handlePurchase}/>
    </section>);
};
export default Home;
